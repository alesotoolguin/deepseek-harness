...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-balance': { kind: 'none', reason: 'Browser-side settings row rendering the balance Remote result; registers nothing model-facing.' },
  'packages/client/ui-git-branch': { kind: 'none', reason: 'Browser-side git branch picker rendering gitBranch Remote results; registers nothing model-facing.' },
  'packages/host/git-branch': { kind: 'none', reason: 'Host-side git branch operations reading repository state; registers nothing model-facing.' },
  'packages/web/web-balance': { kind: 'none', reason: 'The account balance query reads GET /user/balance with the harness credential; it registers nothing model-facing.' },
