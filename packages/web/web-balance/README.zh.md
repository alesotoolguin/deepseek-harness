...
一个仅 Remote 的 Host 服务（`ctx.balance`），从 [`GET /user/balance`](https://api-docs.deepseek.com/api/get-user-balance) 读取 DeepSeek 账户余额，并通过 typert gateway 以 `balance` Remote 命名空间（`api.balance.getBalance()`）暴露给浏览器。
