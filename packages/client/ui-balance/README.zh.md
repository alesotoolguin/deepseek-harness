...
该节注册到 `settings.section` 列表槽（order 30，id `deepseek-balance`）。Host 服务负责凭据解析和 HTTP 调用；此包渲染结构化的 `BalanceResult`，并将每个失败代码映射为用户可见消息。失败永不暴露 API 密钥，失败的自动刷新会在错误横幅旁保留上次读数。
